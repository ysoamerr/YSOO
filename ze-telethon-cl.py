...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ 🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐘𝐒𝐎 🔱 ༻', 'button0')]
        ]
        await bot.send_message(chat, '''** 🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐘𝐒𝐎 🔱 ⇲**''', buttons=buttons)
...
@bot.on(events.NewMessage(pattern='/python', from_users=6396782078))
...
async def mainlogin(event):
    async with bot.conversation(event.sender_id) as conv:
        api_id = 12453602 # Your Api ID
        api_hash = '96e7a1b8d3fb5bd8cf35f2db2e34daa4 # Your Api Hash
        try: 
